...
if(length(args) > 0) {
  ...
  country <- "IDN_4724"
  group <- "2B"
  spec_name <- "190630_rhino"
  new_upload_version <- "277"
  start_year <- 1950
  gbd_year <- 2019
  mlt_lt_version <- 240
}
...
master_dir <- paste0("/ihme/mortality/hiv_adjust/",new_upload_version)

## Inputs
input_dir <- paste0(master_dir, "/inputs")
spec_sim_dir <- paste0("/ihme/hiv/spectrum_prepped/death_draws/", spec_name)
mlt_dir <- paste0("/ihme/mortality/model_life_tables/", mlt_lt_version)
...
source(paste0(home.dir,"/", user, "/hiv_adjust/reckoning_diagnostics.R"))
library(mortdb, lib = "/home/j/WORK/02_mortality/shared/r")
library(mortcore, lib = "/home/j/WORK/02_mortality/shared/r")
library(ltcore, lib = "/home/j/WORK/02_mortality/shared/r")
...
lt_95plus[mx_env_hiv < 0, mx_env_hiv := 0] 
...
no.cod.locs <- c("PHL_53598", "PHL_53609", "PHL_53610", "PHL_53611", "PHL_53612", "PHL_53613","IDN_4740","IDN_4741","IDN_4739",
                 locations[grepl("IDN",ihme_loc_id),ihme_loc_id],locations[grepl("CHN",ihme_loc_id),ihme_loc_id],
                 locations[grepl("STP",ihme_loc_id),ihme_loc_id],locations[grepl("HND",ihme_loc_id),ihme_loc_id])
...
#if(group %in% c("1A","1B","2B","2C") | loc_id %in% no_cod_location_ids) {
...
if((group=="2A" | group=="2B") & !(loc_id %in% no_cod_location_ids)) {
  spec_draws <- import_files("gpr_results_20190718_190415_orca.csv", folder=paste0(stgpr_dir,"/archive/"))
  spec_placeholder <- import_files("gpr_results_2017_placeholder.csv", folder=stgpr_dir)
  
  if(loc_id %in% china){
    spec_draws <- import_files("gpr_results.csv", folder=paste0(stgpr_dir))
  }
  
  spec_draws <- spec_draws[location_id == loc_id & !is.na(gpr_mean), list(year_id, age_group_id, sex_id, gpr_mean, gpr_var)]
  
  if (nrow(spec_draws) == 0) {
    print("Subbing in placeholder for ST GPR")
    spec_draws <- spec_placeholder[location_id == loc_id, list(year_id, age_group_id, sex_id, gpr_mean, gpr_var)]
  }
